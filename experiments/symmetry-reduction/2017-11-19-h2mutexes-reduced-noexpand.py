...
def main(revisions=None):
    ...
    configs = {
        ...
        IssueConfig('translate-reduced-h2mutexes--noexpand-nogoal', ['--translate-options', '--compute-symmetries', '--bliss-time-limit', '300', '--do-not-stabilize-goal', '--only-object-symmetries', '--compute-symmetric-object-sets', '--symmetry-reduced-grounding-for-h2-mutexes', '--h2-mutexes', '--only-positive-literals'], driver_options=['--translate', '--translate-time-limit', '30m', '--translate-memory-limit', '3G']),
    }
    ...
    exp.add_absolute_report_step(attributes=attributes,filter_algorithm=[
        '{}-translate-reduced-h2mutexes-noexpand'.format(REVISION),
        '{}-translate-reduced-h2mutexes--noexpandnogoal'.format(REVISION),
    ])
