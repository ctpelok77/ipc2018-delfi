...
def main(revisions=None):
    ...
    configs = {
        IssueConfig('translate-assert-equal-h2-mutexes', ['--translate-options', '--compute-symmetries', '--stabilize-initial-state', '--only-object-symmetries', '--compute-symmetric-object-sets', '--symmetry-reduced-grounding-for-h2-mutexes', '--expand-reduced-h2-mutexes', '--assert-equal-h2-mutexes', '--h2-mutexes', '--bliss-time-limit', '300', ], driver_options=['--translate', '--translate-time-limit', '30m', '--translate-memory-limit', '3G']),
    }
    ...
    exp.add_absolute_report_step(attributes=attributes,filter_algorithm=[
        '{}-translate-assert-equal-h2-mutexes'.format(REVISION),
